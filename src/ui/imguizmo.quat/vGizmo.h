...
TEMPLATE_TYPENAME_T class virtualGizmoBaseClass {
 public:
  ...
  ////////////////////////////////////////////////////////////////////////////
  virtual void mouse(vgButtons button, vgModifiers mod, bool pressed, T x,
                     T y) {
  ...
  void updateGizmo() {
    ...
    T angle = acos(
        AdotB > T(1)
            ? T(1)
            : (AdotB < -T(1) ? -T(1) : AdotB));  // clamp need!!! corss float is
                                                 // approximate to FLT_EPSILON

    qtStep =
        normalize(angleAxis(angle * tbScale * fpsRatio, axis * rotationVector));
    qtIdle = normalize(angleAxis(angle * tbScale * fpsRatio * qIdleSpeedRatio,
                                 axis * rotationVector));
    qtV = qtStep * qtV;
  }
...
TEMPLATE_TYPENAME_T class virtualGizmo3DClass : public VGIZMO_BASE_CLASS {
 ...
 public:
  ...
  void updateDolly() {
    ...
    dolly -=
        v * dollyScale * (povPanDollyFactor > T(0) ? povPanDollyFactor : T(1));
  }
  ...
  tMat4 getTransform() {
    ...
    // create the translations to move the center of rotation to the origin and
    // back
    trans = translate(tMat4(T(1)), this->rotationCenter);
    invTrans = translate(tMat4(T(1)), -this->rotationCenter);
