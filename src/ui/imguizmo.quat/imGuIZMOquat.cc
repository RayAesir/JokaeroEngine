...
bool imguiGizmo::drawFunc(const char* label, float size) {
  ...
  ImVec2 uv[4];
  ImU32
      col[4];  // buffers to storetransformed vtx & col for PrimVtx & PrimQuadUV
  ...
  auto drawAxes = [&](int side) {
    for (int n = 0; n < 4;
      ...
      for (int arrowAxis = 0; arrowAxis < 3; arrowAxis++) {  // draw 3 axes
        ...
        arrowCoord[arrowAxis] = 1.0f;  // rotate on 3 axis (arrow -> X, Y, Z )
                                       // in base to current arrowAxis
        const float arrowCoordZ = vec3(_q * arrowCoord).z;  //.Rotate

        const int i = (arrowCoordZ > 0)
                          ? 3 - n
                          : n;  // painter algorithm: before farthest
        ...
        for (auto itVtx = ptrVtx->begin(), itNorm = (arrowNorm + i)->begin();
          ...
          for (int h = 0; h < 3; h++) {
            ...
            col[h] = addLightEffect(
                vec4(float(arrowAxis == axisIsX), float(arrowAxis == axisIsY),
  ...
  auto drawComponent = [&](const int idx, const quat& q, ptrFunc func) {
    ...
    for (auto itVtx = ptrVtx->begin(), itNorm = (arrowNorm + idx)->begin();
      ...
      for (int h = 0; h < 3; h++) {
        ...
        vec3 norm = (q * *itNorm++);
#endif
        coord = q * (func(coord) * resizeAxes);  // remodelling Directional
                                                 // Arrow (func) and transforms;

        uv[h] = normalizeToControlSize(coord.x, coord.y);
        // col[h] = addLightEffect(color, float(0xa0)*norm.z+.5f);
        col[h] = addLightEffect(
            vec4(directionColor.x, directionColor.y, directionColor.z, 1.0),
            norm.z, coord.z > 0 ? coord.z : coord.z * .5);
      }
      addTriangle();
    }
  };
  ...
  auto dirArrow = [&](const quat& q, int mode) {
    ...
    if (arrowCoord.z <= 0) {
      for (int i = 0; i < 4; i++) drawComponent(i, q, func);
      if (mode & modeDirPlane) drawPlane();
    } else {
      if (mode & modeDirPlane) drawPlane();
      for (int i = 3; i >= 0; i--) drawComponent(i, q, func);
    }
  };
...
#define CENTER_HELPER_Y -.85f
  ...
  auto drawRotationHelper = [&]() {
    ...
    const float radius = returnSizeFromRatio(0.05f);
