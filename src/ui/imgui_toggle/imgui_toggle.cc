...
// the actual logic takes place in ToggleInternal(), all the Toggle() calls are
// just interfaces for it.
static bool ToggleInternal(const char* label, bool* v, ImGuiToggleFlags flags,
                           float speed, float frame_rounding,
                           float knob_rounding);

bool ImGui::Toggle(const char* label, bool* v) {
  return ::ToggleInternal(
      label, v, ImGuiToggleFlags_Default, ::ToggleDefaultAnimationSpeed,
      ::ToggleFrameRoundingDefault, ::ToggleKnobRoundingDefault);
}

bool ImGui::Toggle(const char* label, bool* v, ImGuiToggleFlags flags) {
  return ::ToggleInternal(label, v, flags, ::ToggleDefaultAnimationSpeed,
                          ::ToggleFrameRoundingDefault,
                          ::ToggleKnobRoundingDefault);
}
...
bool ImGui::Toggle(const char* label, bool* v, ImGuiToggleFlags flags,
  ...
  return ::ToggleInternal(label, v, flags, speed, ToggleFrameRoundingDefault,
                          ::ToggleKnobRoundingDefault);
}

bool ImGui::Toggle(const char* label, bool* v, ImGuiToggleFlags flags,
                   float frame_rounding, float knob_rounding) {
  return ::ToggleInternal(label, v, flags, ::ToggleDefaultAnimationSpeed,
                          frame_rounding, knob_rounding);
}

bool ImGui::Toggle(const char* label, bool* v, ImGuiToggleFlags flags,
                   float speed, float frame_rounding, float knob_rounding) {
  // this overload implies the toggle should be animated.
  flags = (flags & ~ImGuiToggleFlags_Static) | (ImGuiToggleFlags_Animated);
  return ::ToggleInternal(label, v, flags, speed, frame_rounding,
                          knob_rounding);
}
...
static bool ToggleInternal(const char* label, bool* v, ImGuiToggleFlags flags,
  ...
  const float background_rounding =
      frame_rounding >= 0 ? height * frame_rounding : half_height;
  ...
  bool hovered, held;
  bool pressed =
      ImGui::ButtonBehavior(total_bb, id, &hovered, &held, button_flags);
  if (pressed) {
    *v = !(*v);
  ...
  float t = mixed_value ? 0.5f : (*v ? 1.0f : 0.0f);

  if (is_animated && g.LastActiveId == id) {
    float speed_mul = 1.0f / (speed > 0 ? speed : 1.0f);
    float t_anim = ImSaturate(g.LastActiveIdTimer /
                              (speed_mul * ToggleAnimationSpeedScaler));
    t = *v ? (t_anim) : (1.0f - t_anim);
  }

  // FIXME: toggle knobs could get their own theme colors rather than using text
  // & buttons.
  ...
  if (g.HoveredId == id) {
    background_color = ImGui::GetColorU32(
        is_animated ? ImLerp(color_frame_off_hover, color_frame_on_hover, t)
                    : (*v ? color_frame_on_hover : color_frame_off_hover));
  } else {
    background_color = ImGui::GetColorU32(
        is_animated ? ImLerp(color_frame_off, color_frame_on, t)
  ...
  draw_list->AddRectFilled(frame_min, frame_max, background_color,
                           background_rounding);
  ...
  if (is_circle_knob) {
    const ImVec2 knob_center =
        ImVec2(pos.x + radius + t * (width - double_radius), pos.y + radius);
    ...
    draw_list->AddRectFilled(knob_min, knob_max, color_knob,
                             knob_rounded_height);
  ...
  const ImVec2 label_pos = ImVec2(toggle_bb.Max.x + style.ItemInnerSpacing.x,
                                  toggle_bb.Min.y + style.FramePadding.y);

  if (g.LogEnabled) {
    ImGui::LogRenderedText(&label_pos, mixed_value ? "[~]"
                                       : *v        ? "[x]"
                                                   : "[ ]");
  }
