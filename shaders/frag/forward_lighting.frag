...
layout(binding = 6) uniform sampler2D texHbao;
layout(binding = 7) uniform usampler2D texGtao;
...
void main() {
...
#ifdef ALPHA
  if (pbr.alpha <= mat.alpha_threshold) discard;
#endif
  ...
  if (uEnableHbao == 1) final_color *= texelFetch(texHbao, pixel_coords, 0).r;
  if (uEnableGtao == 1) {
    uint packed_value = texelFetch(texGtao, pixel_coords, 0).r;
